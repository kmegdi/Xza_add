...
def update_token():
    ...
    while True:
        ...
        if new_token:
            ...
            app.logger.warning("🔁 فشل في تحديث التوكن. سيتم إعادة المحاولة.")
        time.sleep(1)  # ⏱️ تحديث كل ثانية واحدة
...
def send_friend():
    try:
        ...
        app.logger.error("❌ خطأ في /cntrol_panel;: %s", e, exc_info=True)
